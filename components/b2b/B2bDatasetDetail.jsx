...
const B2bDatasetDetail = ({ id, country, category, initialDataset = null }) => {
    ...
    const handleSampleDownload = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                // Backend has data — download the CSV blob
                ...
                // 3. Fallback: Generate masked Excel from /api/merged/data
                ...
                } catch (e) { console.error("Fallback data fetch failed:", e); }
                ...
                if (country && category) {
                    try {
                        ...
                        const mergedDataset = {
                            ...
                            linkedinCount: catInfo?.linkedin !== undefined ? catInfo.linkedin : (hasLinkedin ? Math.floor(totalRecords * 0.6) : 0),
                            totalLinkedin: catInfo?.linkedin !== undefined ? catInfo.linkedin : (hasLinkedin ? Math.floor(totalRecords * 0.6) : 0),
                            facebookCount: catInfo?.facebook !== undefined ? catInfo.facebook : (hasFacebook ? Math.floor(totalRecords * 0.65) : 0),
                            totalFacebook: catInfo?.facebook !== undefined ? catInfo.facebook : (hasFacebook ? Math.floor(totalRecords * 0.65) : 0),
                            instagramCount: catInfo?.instagram !== undefined ? catInfo.instagram : (hasInstagram ? Math.floor(totalRecords * 0.5) : 0),
                            totalInstagram: catInfo?.instagram !== undefined ? catInfo.instagram : (hasInstagram ? Math.floor(totalRecords * 0.5) : 0),
                            twitterCount: catInfo?.twitter !== undefined ? catInfo.twitter : (hasTwitter ? Math.floor(totalRecords * 0.3) : 0),
                            totalTwitter: catInfo?.twitter !== undefined ? catInfo.twitter : (hasTwitter ? Math.floor(totalRecords * 0.3) : 0),
                            tiktokCount: catInfo?.tiktok !== undefined ? catInfo.tiktok : (hasTiktok ? Math.floor(totalRecords * 0.2) : 0),
                            totalTiktok: catInfo?.tiktok !== undefined ? catInfo.tiktok : (hasTiktok ? Math.floor(totalRecords * 0.2) : 0),
